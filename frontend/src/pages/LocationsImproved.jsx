...
const LocationsImprovedPage = () => {
  ...
  const [modalType, setModalType] = useState("college"); // college | department | room
        ...
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            <Avatar sx={{ bgcolor: alpha(theme.palette.success.main, 0.2), width: 32, height: 32 }}>
              <BusinessCenter sx={{ color: theme.palette.success.main, fontSize: 18 }} />
            </Avatar>
            <Box>
  ...
  return (
    <DashboardLayout>
      <Box sx={{ p: 3 }}>
        ...
        {/* Add College Button */}
        <Button variant="contained" startIcon={<Add />} onClick={() => openCollegeModal("add")} sx={{ mb: 3 }}>
          Create New College
        </Button>
      ...
      <Dialog open={showModal} onClose={() => setShowModal(false)} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
        ...
        <DialogContent sx={{ py: 3, px: 3 }}>
          <Stack spacing={2.5} sx={{ mt: 1 }}>
            ...
            {(modalType === "department" || modalType === "room") && (
              <>
                <Grid container spacing={2}>
                  <Grid item xs={6}>
                    <TextField
...
export default LocationsImprovedPage;
